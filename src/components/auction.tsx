...
export function CarStage({ car, lotNo, hideMarketPrice }: { car: Car; lotNo: number; hideMarketPrice: boolean }) {
  return <div>
    <div style={{ position: 'relative', aspectRatio: '16 / 10', width: '100%', background: color.panel2, borderRadius: 6, overflow: 'hidden', border: `1px solid ${color.line}` }}>
      {car.imageUrl ? <img src={car.imageUrl} alt={car.nameAr} loading="eager" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', position: 'relative', zIndex: 1 }} /> : null}
      <div style={{ position: 'absolute', inset: 0, zIndex: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontFamily: font.display, fontWeight: 900, fontSize: 'clamp(40px, 12vw, 76px)', lineHeight: 1, color: tierColor[car.tier], opacity: 0.16, userSelect: 'none' }}>{TIER_AR[car.tier]}</span></div>
      <span dir="ltr" style={{ position: 'absolute', insetInlineStart: 10, insetBlockStart: 10, zIndex: 2, fontFamily: font.num, fontSize: 11, fontWeight: 600, letterSpacing: '0.18em', color: color.text, background: 'rgba(13,17,23,.82)', border: `1px solid ${color.line}`, borderRadius: 4, padding: '4px 9px' }}>LOT {String(lotNo).padStart(3, '0')}</span>
    </div>
